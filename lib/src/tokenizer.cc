...
namespace hocon {
    ...
    shared_token token_iterator::pull_comment(char first_char) {
        ...
        if (first_char == '/') {
            ...
            if (discard != '/') {
                throw config_exception(fmt::format("called pull_comment() but // not seen"));
            }
            double_slash = true;
        }
        ...
        try {
            ...
            for (char character : result) {
                if (not_in_unquoted_text.find(character) != string::npos) {
                    throw config_exception(fmt::format("Line {0}: Reserved character '{1}' not allowed outside quotes", std::to_string(_line_number), character));
                }
            }
    ...
    void token_iterator::pull_escape_sequence(string& parsed, string& original) {
        if (!*_input) {
            throw config_exception(fmt::format("End of input but backslash in string had nothing after it"));
        }
        ...
        switch (escaped) {
            ...
            case 'u': {
                ...
                for (int i = 0; i < 4; i++) {
                    if (!*_input) {
                        throw config_exception(fmt::format("End of input but expecting 4 hex digits for \\uXXXX escape"));
                    }
                    utf[i] = _input->get();
                }
                original += string(utf);
                ...
                break;
            default:
                throw config_exception(fmt::format("backslash followed by {0}, this is not a valid escape sequence. (Quoted strings use JSON escaping, so use double-backslash \\\\ for literal backslash)", string(1, escaped)));
        }
    }
    ...
    void token_iterator::append_triple_quoted_string(string& parsed, string& original) {
        ...
        while (true) {
            ...
            if (c == '"') {
                ...
                if (!*_input) {
                    throw config_exception(fmt::format("End of input but triple-quoted string was still open"));
                } else if (c == '\n') {
                    _line_number++;
    ...
    shared_token token_iterator::pull_quoted_string() {
        ...
        while (true) {
            if (!*_input) {
                throw config_exception(fmt::format("End of input but string quote was still open"));
            }
            ...
            if (c == '\\') {
                ...
                throw config_exception(fmt::format("Line {0}: JSON does not allow unescaped {1} in quoted strings, use a backslash escape", std::to_string(_line_number), string(1, c)));
            } else {
                result += c;
                original += c;
            }
    ...
    shared_token const& token_iterator::pull_plus_equals() {
        char c = _input->get();
        if (c != '=') {
            throw config_exception(fmt::format("'+' not followed by '=', '{0}' not allowed after '+'", string(1, c)));
        }
    ...
    shared_token token_iterator::pull_next_token(whitespace_saver& saver) {
        ...
        if (!*_input) {
            ...
            if (start_of_comment(c)) {
                ...
                if (t == nullptr) {
                    if (first_number_chars().find(c) != string::npos) {
                        ...
                        throw config_exception(fmt::format("Reserved character '{0}' is not allowed outside quotes", string(1, c)));
                    } else {
                        _input->putback(c);
            ...
            if (t == nullptr) {
                throw config_exception(fmt::format("Failed to generate next token"));
            }
    ...
    shared_token token_iterator::next() {
        ...
        if (_tokens.empty() && t != tokens::end_token()) {
            ...
            if (_tokens.empty()) {
                throw config_exception(fmt::format("Tokens queue should not be empty here"));
            }
        }
        return t;
    }
