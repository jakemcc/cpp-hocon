...
namespace hocon {
    ...
    void config_delayed_merge::render(vector<shared_value> const& stack, string& s, int indent_value, bool at_root, string const& at_key, config_render_options options) {
        ...
        if (comment_merge) {
            s += fmt::format("# unresolved merge of {0} values follows (\n", to_string(stack.size()));
            if (at_key == "") {
                indent(s, indent_value, options);
                s += fmt::format("# this unresolved merge will not be parseable because it's at the root of the object =\n");
                indent(s, indent_value, options);
                s += fmt::format("# the HOCON format has no way to list multiple root objects in a single file \n");
            }
        }
        ...
        for (auto const& v : reversed) {
            if (comment_merge) {
                indent(s, indent_value, options);
                if (at_key == "") {
                    s += fmt::format("#     unmerged value {0} for key {1} from ", to_string(i), hocon::render_json_string(at_key));
                } else {
                    s += fmt::format("#     unmerged value {0} from ", to_string(i));
                }
                i++;
        ...
        if (comment_merge) {
            indent(s, indent_value, options);
            s += fmt::format("# ) end of unresolved merge\n");
        }
    }
