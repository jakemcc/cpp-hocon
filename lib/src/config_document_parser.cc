...
namespace hocon { namespace config_document_parser {
    ...
    shared_token parse_context::next_token() {
        ...
        if (_flavor == config_syntax::JSON) {
            if (t->get_token_type() == token_type::UNQUOTED_TEXT && !is_unquoted_whitespace(t)) {
                throw parse_error(fmt::format("Token not allowed in valid JSON: '{0}'", t->token_text()));
    ...
    shared_node_value parse_context::consolidate_values(shared_node_list &nodes) {
        ...
        while (t) {
            ...
            if (v == nullptr) {
                throw config_exception(fmt::format("no value"));
            }
                                                    ...
                                                    path *last_path)
    {
        ...
        if (bad_token == tokens::end_token()->to_string()) {
            ...
            if (!prev_field_name.empty()) {
                part = fmt::format("{0} (if you intended '{1}' to be part of a value, instead of a key, try adding double quotes around the whole value", message, prev_field_name);
            } else {
                return message;
            }
        } else {
            if (!prev_field_name.empty()) {
                part = fmt::format("{0} (if you intended {1} to be part of the value for '{2}', try enclosing the value in double quotes",  message, bad_token, prev_field_name);
            } else {
                part = fmt::format("{0} (if you intended {1} to be part of a key or string value, try enclosing the key or value in double quotes", message, bad_token);
            }
        }

        if (inside_equals) {
            return fmt::format("{0}, or you may be able to rename the file .properties rather than .conf)", part);
        } else {
            return part + ")";
        }
    }
    ...
    shared_node_value parse_context::parse_value(shared_token t) {
        ...
        if (t->get_token_type() == token_type::VALUE || t->get_token_type() == token_type::UNQUOTED_TEXT ||
            ...
            throw parse_error(add_quote_suggestion(t->to_string(),
                                                   fmt::format("Expecting a value but got wrong token: {0}", t->to_string())));
        }

        if (_equals_count != starting_equals_count) {
            throw config_exception(fmt::format("Bug in config parser: unbalanced quals count"));
        }
    ...
    shared_ptr<config_node_path> parse_context::parse_key(shared_token token) {
        if (_flavor == config_syntax::JSON) {
            if (tokens::is_value_with_type(token, config_value::type::STRING)) {
                ...
                throw parse_error(fmt::format("Expecting close brace or a field name here, got {0}", token->to_string()));
            }
        } else {
            ...
            if (expression.empty()) {
                throw parse_error(fmt::format("expecting a close brace or a field name here, got {0}", t->to_string()));
            }
    ...
    shared_ptr<config_node_include> parse_context::parse_include(shared_node_list& children) {
        ...
        if (t->get_token_type() == token_type::UNQUOTED_TEXT) {
            ...
            if (kind_text == "url(") {
                ...
                throw parse_error(fmt::format("expecting include parameter to be quoted filename, file(), classpath(), or url(). No spaces are allowed before the open paren. Not expecting: {0}", t->to_string()));
            }
            ...
            if (!tokens::is_value_with_type(t, config_value::type::STRING)) {
                throw parse_error(fmt::format("expecting a quoted string inside file(), classpath(), or url(), rather than {0}", t->to_string()));
            }
            ...
            if (t->token_text() != ")") {
                throw parse_error(fmt::format("expecting a close parentheses ')' here, not: {0}", t->to_string()));
            }
            ...
            throw parse_error(fmt::format("include keyword is not followed by a quoted string, but by: {0}", t->to_string()));
        }
    }
        ...
        while (true) {
            ...
            if (t->get_token_type() == token_type::CLOSE_CURLY) {
                ...
                if (_flavor == config_syntax::CONF && after_key->get_token_type() == token_type::OPEN_CURLY) {
                    ...
                    if (!is_key_value_separator(after_key)) {
                        ...
                        fmt::format("Key '{0}' may not be followed by token: {1}", key_path->render(), after_key->to_string())));
                    }
                ...
                if (remaining.empty()) {
                    ...
                    if (!emplace.second) {
                        ...
                        if (_flavor == config_syntax::JSON) {
                            throw parse_error(fmt::format("JSON does not allow duplicate fields: '{0}' was already seen", key));
                        }
                        emplace.first->second = true;
                    }
                } else {
                    if (_flavor == config_syntax::JSON) {
                        throw config_exception(fmt::format("somehow got multi-element path in JSON mode"));
                    }
            ...
            if (check_element_separator(object_nodes)) {
                ...
                if (t->get_token_type() == token_type::CLOSE_CURLY) {
                    ...
                    throw parse_error(add_quote_suggestion(t->to_string(),
                                                           fmt::format("Expecting close brace '}}' or a comma, got {0}", t->to_string()), last_inside_equals, last_path));
                } else {
                    if (t->get_token_type() == token_type::END) {
                        ...
                        throw parse_error(add_quote_suggestion(t->to_string(),
                            fmt::format("Expecting end of input or a comma, got {0}", t->to_string()), last_inside_equals, last_path));
                    }
                }
            }
        }
    ...
    shared_ptr<config_node_complex_value> parse_context::parse_array() {
        ...
        if (next_value) {
            ...
            if (t->get_token_type() == token_type::CLOSE_SQUARE) {
                ...
                throw parse_error(fmt::format("List should have ']' or a first element after the '[', instead had token: {0} (if you want {1} to be part of a string value, then double quote it)",  t->to_string(), t->to_string()));
            }
        }
        ...
        while (true) {
            ...
            if (check_element_separator(children)) {
                ...
                if (t->get_token_type() == token_type::CLOSE_SQUARE) {
                    ...
                    throw parse_error(fmt::format("List should have ended with ']' or had a comma, instead had token: {0} (if you want {1} to be part of a string value, then double quote it)", t->to_string(), t->to_string()));
                }
            }
            ...
            if (next_value) {
                ...
                if (is_valid_array_element(t)) {
                    ...
                    throw parse_error(fmt::format("List should have had a new element after a comma, instead had token: {0} (if you want the comma or {1} to be part of a string value, then double quote it)", t->to_string(), t->to_string()));
                }
            }
        }
    }
    ...
    shared_ptr<config_node_root> parse_context::parse() {
        ...
        if (t->get_token_type() == token_type::START) {
            ...
            throw config_exception(fmt::format("token stream did not begin with START, had {0}", t->to_string()));
        }
        ...
        if (t->get_token_type() == token_type::OPEN_CURLY || t->get_token_type() == token_type::OPEN_SQUARE) {
            ...
            if (_flavor == config_syntax::JSON) {
                if (t->get_token_type() == token_type::END) {
                    throw parse_error(fmt::format("empty document"));
                } else {
                    throw parse_error(fmt::format("Document must have an object or array at root, unexpected token: {0}", t->to_string()));
                }
            } else {
        ...
        if (t->get_token_type() == token_type::END) {
            ...
            throw parse_error(fmt::format("Document has trailing tokens after first object or array: {0}", t->to_string()));
        }
    }
    ...
    shared_node_value parse_context::parse_single_value() {
        ...
        if (t->get_token_type() == token_type::IGNORED_WHITESPACE || t->get_token_type() == token_type::NEWLINE ||
            ...
            throw parse_error(fmt::format("The value from with_value_text cannot have leading or trailing newlines, whitespace, or comments"));
        }
        if (t->get_token_type() == token_type::END) {
            throw parse_error(fmt::format("Empty value"));
        }
        if (_flavor == config_syntax::JSON) {
            ...
            if (t->get_token_type() == token_type::END) {
                ...
                throw parse_error(fmt::format("Parsing JSON and the value set in with_value_text was either a concatenation or had trailing whitespace, newlines, or comments"));
            }
        } else {
            put_back(t);
